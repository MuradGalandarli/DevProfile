...
import { BackendExpertiseSection } from '../components/common/BackendExpertiseSection'
import { FrontendExpertiseSection } from '../components/common/FrontendExpertiseSection'
import { EngineeringApproachSection } from '../components/common/EngineeringApproachSection'
...
export function HomePage() {
  return (
    <>
    <Seo title="Murad Galandarli | Software Developer" description="Software Developer specializing in C#, ASP.NET Core, React and TypeScript." canonicalPath="/" />
    <section className="mx-auto grid w-[min(100%-2rem,72rem)] items-center gap-12 py-16 md:py-24 lg:min-h-[calc(100vh-10rem)] lg:grid-cols-[1.08fr_0.92fr] lg:py-16">
      <div className="max-w-2xl">
        ...
        <div className="mt-9 flex flex-wrap gap-3">
          <Link className="inline-flex items-center gap-2 rounded-lg bg-[var(--color-accent)] px-4 py-2.5 text-sm font-semibold text-[var(--color-on-accent)] no-underline transition-transform hover:-translate-y-0.5 hover:bg-[var(--color-accent-strong)]" to="/projects">View Projects <ArrowRight aria-hidden="true" size={17} /></Link>
      ...
      <div className="relative mx-auto w-full max-w-lg" role="img" aria-label="Technology code illustration">
        ...
        <div className="overflow-hidden rounded-2xl border border-[var(--color-line)] bg-[var(--color-surface)] shadow-2xl shadow-black/20">
          ...
          <div className="space-y-4 p-6 font-mono text-sm leading-6 sm:p-8">
            ...
            <p className="m-0"><span className="text-[var(--color-code-keyword)]">const</span> <span className="text-[var(--color-accent)]">stack</span> <span className="text-[var(--color-muted)]">=</span> <span className="text-[var(--color-code-punct)]">{'{'}</span></p>
            <div className="space-y-1 pl-5 text-[var(--color-ink)]"><p className="m-0">backend: <span className="text-[var(--color-code-string)]">'ASP.NET Core'</span>,</p><p className="m-0">frontend: <span className="text-[var(--color-code-string)]">'React'</span>,</p><p className="m-0">language: <span className="text-[var(--color-code-string)]">'TypeScript'</span></p></div>
            <p className="m-0 text-[var(--color-code-punct)]">{'}'};</p>
    ...
    <FrontendExpertiseSection />
    <SkillsSection />
    <EngineeringApproachSection />
    <ExperienceSection />
    <ProjectsSection />
    <div className="mx-auto w-[min(100%-2rem,72rem)]"><EducationSection /><ProfileStrengthsSection /></div>
    <CallToActionSection />
    </>
  )
}
