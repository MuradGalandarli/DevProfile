...
export function ProjectsPage() {
  ...
  return (
    <section className="mx-auto w-[min(100%-2rem,72rem)] py-16 sm:py-24">
      ...
      {visibleProjects.length ? <div className="mt-10 grid gap-5 md:grid-cols-2">{visibleProjects.map((project) => <ProjectCard key={project.id} project={project} />)}</div> : <div className="mt-10 rounded-2xl border border-dashed border-[var(--color-line)] bg-[var(--color-surface)] px-6 py-16 text-center"><h2 className="m-0 text-xl font-semibold text-[var(--color-ink)]">No matching projects</h2><p className="mx-auto mt-3 mb-0 max-w-md text-sm leading-6 text-[var(--color-muted)]">Try a different search term or change the selected category to explore the available project entries.</p><button className="mt-6 rounded-lg border border-[var(--color-line)] px-3 py-2 text-sm font-semibold text-[var(--color-ink)] hover:border-[var(--color-accent)] hover:text-[var(--color-accent)]" type="button" onClick={() => { setActiveFilter('all'); setSearchTerm('') }}>Clear filters</button></div>}
    </section>
  )
}
