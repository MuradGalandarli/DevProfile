...
export function ProjectDetailsPage() {
  ...
  if (!project) {
    return <>
    <Seo title="Project Not Found | Murad Galandarli" description="The requested project could not be found in the portfolio." canonicalPath="/projects" />
    <section className="mx-auto w-[min(100%-2rem,72rem)] py-16 sm:py-24"><ErrorState eyebrow="Project unavailable" title="This project was not found." message="The project identifier may be incorrect, or this case study is not currently included in the local portfolio data." action={<Link className="inline-flex items-center gap-2 rounded-lg border border-[var(--color-line)] px-4 py-2.5 text-sm font-semibold text-[var(--color-ink)] no-underline transition-colors hover:border-[var(--color-accent)] hover:text-[var(--color-accent)]" to="/projects"><ArrowLeft aria-hidden="true" size={17} /> Back to Projects</Link>} /></section>
    </>
  }

  return (
    <>
    <Seo title={`${project.title} | Murad Galandarli | Software Developer`} description={project.description} canonicalPath={`/projects/${project.id}`} />
